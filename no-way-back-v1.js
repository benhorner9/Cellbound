...
function renderGate(){
  ...
  function frame(now){if(!active)return;const dt=Math.min(60,now-last);last=now;pos+=dir*dt*.05;if(pos>=100){pos=100;dir=-1}else if(pos<=0){pos=0;dir=1}const needle=root.querySelector('[data-tension-needle]');if(needle)needle.style.left=pos+'%';requestAnimationFrame(frame)}
