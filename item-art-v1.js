...
function enhanceCrafting(root){
 ...
 scope.querySelectorAll?.('.crafted-card').forEach(function(card){
  ...
  if(recipe?.output?.category==='consumable'){b.insertAdjacentHTML('beforebegin',consumableHTML(recipe.output.key,48,'crafted-item-art'));card.dataset.itemArtDone='1'}
 });
...
function enhanceAll(root){installDataAdapters();enhancePvp(root);enhanceCollections(root);enhanceCrafting(root);enhanceTrade(root)}
