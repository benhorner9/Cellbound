...
function decorate(){
  ...
  root.querySelectorAll('.cb-current-item').forEach(card=>{
    ...
    card.classList.add(`cb-tier-${item.tier}`);replaceIcon(card.firstElementChild,item,52);
  });
  root.querySelectorAll('.cb-slot-options button').forEach(card=>{
    const item=canonicalFrom(card);if(!item)return;
    card.classList.add(`cb-tier-${item.tier}`);replaceIcon(card.firstElementChild,item,48);
