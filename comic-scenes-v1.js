...
function panelMarkup(panel,index,progressive=false){
  ...
  return'<article class="cbcomic-panel kind-'+kind+' panel-'+(index+1)+' '+(p.wide?'wide ':'')+(p.artwork?'has-art ':'')+(progressive?'reveal-pending':'reveal-visible')+'" data-panel="'+index+'">'+art+'<div class="cbcomic-panel-art" aria-hidden="true"><i></i><i></i><i></i><em></em></div>'+sigil+speaker+caption+'</article>';
}
...
function show(config={}){
  ...
  return new Promise(resolve=>{
    ...
    const progressive=Boolean(config.progressive);
    const panelOnly=Boolean(config.panelOnly);
    const allowSkip=config.allowSkip!==false;
    let revealed=progressive?0:panels.length;

    root.dataset.theme=config.theme||'zeltira';
    root.innerHTML='<section class="cbcomic-shell '+(panelOnly?'panel-only ':'')+(progressive?'progressive':'')+'" role="dialog" aria-modal="true" aria-label="'+esc(config.title||'Story scene')+'">'+
      ...
      '<div class="cbcomic-page">'+panels.map((p,i)=>panelMarkup(p,i,progressive)).join('')+'</div>'+
      (config.hideParty?'':scenePartyMarkup())+
      '<footer class="cbcomic-dialogue">'+
        (panelOnly?'':'<div class="cbcomic-speaker">'+(config.speakerPortrait?'<img src="'+esc(config.speakerPortrait)+'" alt="">':'<span>'+esc(config.speakerMark||String(config.speaker||'NPC').split(/\s+/).map(x=>x[0]).slice(0,2).join(''))+'</span>')+'<div><small>'+esc(config.speakerRole||'')+'</small><b>'+esc(config.speaker||'Narrator')+'</b></div></div><div class="cbcomic-line"><p id="cbcomicLine">'+esc(config.line||'')+'</p><div id="cbcomicReply" class="cbcomic-reply" hidden></div></div>')+
        '<div class="cbcomic-controls">'+(panelOnly?'<div id="cbcomicReply" class="cbcomic-reply cbcomic-choice-reply" hidden></div>':'')+'<div id="cbcomicChoices" class="cbcomic-choices">'+choices.map(choiceMarkup).join('')+'</div><div class="cbcomic-actions">'+(allowSkip?'<button type="button" data-comic-skip>'+esc(config.skipLabel||'SKIP')+'</button>':'')+'<button type="button" class="primary" data-comic-continue>'+esc(progressive?(config.nextLabel||'NEXT →'):(config.continueLabel||'CONTINUE →'))+'</button></div></div>'+
      '</footer></section>';

    const cont=root.querySelector('[data-comic-continue]');
    const choicesWrap=root.querySelector('#cbcomicChoices');
    const syncProgress=()=>{
      const done=!progressive||revealed>=panels.length;
      root.querySelectorAll('[data-panel]').forEach((el,i)=>{
        const visible=!progressive||i<revealed;
        el.classList.toggle('reveal-pending',!visible);
        el.classList.toggle('reveal-visible',visible);
        el.classList.toggle('reveal-current',progressive&&visible&&i===revealed-1);
      });
      if(choicesWrap)choicesWrap.hidden=progressive&&!done;
      if(cont){
        cont.textContent=done?(config.continueLabel||'CONTINUE →'):(config.nextLabel||'NEXT →');
        cont.disabled=Boolean(done&&choices.length&&!selected);
      }
    };

    root.querySelector('[data-comic-skip]')?.addEventListener('click',()=>finish({choiceId:selected,skipped:true}));
    cont?.addEventListener('click',()=>{
      if(progressive&&revealed<panels.length){revealed++;syncProgress();return}
