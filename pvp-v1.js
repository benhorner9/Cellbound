...
const MODE_DEFS={
  'capture-the-flag':{name:'Capture the Flag',short:'CTF',icon:'⚑',description:'Break the enemy line, steal their Cell Standard and escort it home while your defenders protect yours.'},
  'king-of-the-hill':{name:'King of the Hill',short:'KOTH',icon:'◇',description:'Fight for the central Cell node. Hold the zone long enough to drain the enemy score and secure the field.'}
};
...
function gearPiece(tier,slot){
  ...
  return {id:`pvp-t${tier}-${String(slot).toLowerCase()}`,name:`${t.name} ${slot}`,tier,slot,pvpOnly:true,pvpPower:Math.round(t.power*m),pvpDefence:Math.round(t.defence*m),controlResistance:Math.round(t.control*m),acquiredAt:nowIso()}
}
