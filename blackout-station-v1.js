...
async function startBoss(){
 if(!run)return;await window.CellboundExpeditionPresentation?.room?.('blackout-station',{title:'Generator Hall · Dr. Vex Calder',index:1,total:2,kind:'FINAL BOSS',force:true,long:true});drawCombat();window.CellboundFX?.boss?.('Dr. Vex Calder','Restore the grid. Survive the role circuits.');const tok=token,C=window.CellboundCombatStandard;if(!C?.simulate){setStatus('Combat Reborn unavailable');feed('Combat standard gateway unavailable.');return}
 try{
...
function bsCombatAnalysisHTML(metrics,timeMs){
 ...
 return '<section class="cbr-analysis"><div class="cbr-analysis-head"><div><small>COMBAT REBORN · RUN ANALYSIS</small><h4>Grid execution and boss performance.</h4></div></div><div class="cbr-analysis-grid"><article><span>COMBAT TIME</span><b>'+time+'</b></article><article><span>DAMAGE</span><b>'+Math.round(damage).toLocaleString()+'</b></article><article><span>HEALING</span><b>'+Math.round(healing).toLocaleString()+'</b></article><article><span>PUZZLE MOVES</span><b>'+Number(run?.moves||0)+'</b></article><article><span>DEATHS</span><b>'+Number(metrics?.deaths||0)+'</b></article><article><span>FAILED MECHANICS</span><b>'+Number(metrics?.mechanicsFailed||0)+'</b></article></div><div class="cbr-analysis-list">'+rows+'</div></section>'
}
async function complete(){
 ...
 e.innerHTML='<div class="cb2d-loot-wrap"><header class="cb2d-loot-head"><div><small>BLACKOUT STATION · '+esc(mode==='cellbound'?'CELLBOUND+'+tier:mode.toUpperCase())+' · CLEARED</small><h3>Grid Secured</h3><p>The generator hall falls silent. The station is powered, Calder is down, and the recovered equipment has been secured.</p></div><div class="cb2d-loot-complete">✓<span>DUNGEON<br>COMPLETE</span></div></header><div class="cb2d-loot-currency"><article><span>GOLD</span><b>+'+gold+'</b><small>Added to Guild treasury</small></article><article><span>RENOWN</span><b>+'+renown+'</b><small>Guild reputation earned</small></article><article><span>PARTY XP</span><b>+'+XP+'</b><small>Each adventurer</small></article><article><span>CELL SHARDS</span><b>+'+shards+'</b><small>Recovered from the grid</small></article><article><span>RUN SCORE</span><b>'+score+'</b><small>'+Math.round(timeMs/1000)+'s expedition</small></article></div>'+(unlocks?'<section class="eg-unlock-panel"><small>PROGRESSION</small><div>'+unlocks+'</div></section>':'')+'<section class="cb2d-loot-section"><div class="cb2d-loot-title"><span>GEAR ACQUIRED</span><small>Stored automatically in the Guild Bank</small></div><div class="cb2d-loot-gear">'+(gear?bsLootGearCard(gear):'<div class="cb2d-loot-empty">No equipment recovered.</div>')+'</div></section><section class="cb2d-loot-section cb2d-xp-section"><div class="cb2d-loot-title"><span>PARTY EXPERIENCE</span><small>Every member of the active five gains experience from the clear</small></div><div class="cb2d-xp-grid">'+gains.map(bsXpCard).join('')+'</div></section>'+(overrideDrop?'<section class="cb2d-loot-section bs-override-drop-section"><div class="cb2d-loot-title"><span>RARE UTILITY DROP</span><small>10% chance · separate from equipment loot</small></div><article class="bs-override-drop-card">'+gridOverrideArt(72)+'<div><small>RARE · UTILITY</small><h4>Grid Override Module</h4><p>5 / 5 uses · Automatically restores the Blackout Station grid after your first manual clear.</p><em>TRADEABLE · SENT TO GUILD BANK</em></div></article></section>':'')+'<section class="bs-run-summary"><article><b>'+run.moves+'</b><span>Puzzle moves</span></article><article><b>'+deaths+'</b><span>Deaths</span></article><article><b>'+failed+'</b><span>Failed mechanics</span></article></section>'+bsCombatAnalysisHTML(metrics,timeMs)+'<footer class="cb2d-loot-actions"><button data-bs-bank>VIEW GUILD BANK</button><button class="primary" data-bs-return>RETURN TO DUNGEONS →</button></footer></div>';
