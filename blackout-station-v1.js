...
function renderCard(){
 ...
 '<article class="intel-card"><div class="intel-card-head"><b>Combat Rule</b><span>POSITIONAL</span></div><p>The circles use live arena positions. The party must reach the correct zone before the shockwave hits.</p></article>'+
 '<article class="intel-card"><div class="intel-card-head"><b>Expedition Record</b><span>'+clears+' clear'+(clears===1?'':'s')+'</span></div><p>'+(clears?'The station is restored. Calder remains available for repeat runs.':'No successful restoration has been recorded yet.')+'</p></article>'+
 '</div></aside></div>';
...
function awardXp(){
 ...
 return party().map(c=>{
  const beforeLevel=Math.min(cap,Math.max(1,Number(c.level)||1)),beforeXp=beforeLevel>=cap?0:Math.max(0,Number(c.xp)||0),beforeNeed=hpNeed(beforeLevel);
  let level=beforeLevel,xp=beforeLevel>=cap?0:beforeXp+XP,levels=0;
  while(level<cap&&xp>=hpNeed(level)){xp-=hpNeed(level);level++;levels++}
  if(level>=cap){level=cap;xp=0}
  c.level=level;c.xp=xp;if(levels)c.talent=(Number(c.talent)||0)+levels;
  return{name:c.name,beforeLevel,beforeXp,beforeNeed,afterLevel:level,afterXp:xp,afterNeed:hpNeed(level),level,xp,levels,capped:level>=cap}
 })
}
...
function bsXpCard(x){
 ...
 return '<article class="cb2d-xp-card" data-bs-xp data-start="'+start.toFixed(2)+'" data-end="'+end.toFixed(2)+'" data-levels="'+Number(x.levels||0)+'"><div class="cb2d-xp-avatar">'+esc(portrait)+'</div><div class="cb2d-xp-copy"><div><span><b>'+esc(x.name)+'</b><small>Level '+x.beforeLevel+(x.afterLevel!==x.beforeLevel?' → '+x.afterLevel:'')+'</small></span>'+(x.capped?'<em class="cb2d-level-up">MAX LEVEL</em>':x.levels?'<em class="cb2d-level-up">LEVEL UP</em>':'<em>+'+XP+' XP</em>')+'</div><div class="cb2d-xp-bar"><i style="width:'+start.toFixed(2)+'%"></i></div><p><span>'+x.beforeXp+' / '+x.beforeNeed+' XP</span><strong>+'+XP+' XP</strong><span>'+x.afterXp+' / '+x.afterNeed+' XP</span></p></div></article>'
}
function bsAnimateXp(rootEl){
 [...(rootEl?.querySelectorAll('[data-bs-xp]')||[])].forEach((row,index)=>{const bar=row.querySelector('.cb2d-xp-bar i'),end=Number(row.dataset.end)||0,levels=Number(row.dataset.levels)||0;if(!bar)return;setTimeout(()=>{if(!levels){bar.style.width=end+'%';return}bar.style.width='100%';setTimeout(()=>{row.classList.add('levelled');bar.style.transition='none';bar.style.width='0%';requestAnimationFrame(()=>requestAnimationFrame(()=>{if(!bar.isConnected)return;bar.style.transition='width .8s cubic-bezier(.2,.75,.25,1)';bar.style.width=end+'%'}))},760)},220+index*90)})
}
function bsCombatAnalysisHTML(metrics,timeMs){
 const result=run?.result||{},summary=result.summary||{},players=Array.isArray(summary.players)?summary.players:[],damage=Object.values(run?.damage||{}).reduce((n,v)=>n+(Number(v)||0),0),healing=Object.values(run?.healing||{}).reduce((n,v)=>n+(Number(v)||0),0),secs=Math.max(0,Math.round((Number(result.durationMs)||Number(timeMs)||0)/1000)),time=(Math.floor(secs/60)?Math.floor(secs/60)+'m ':'')+(secs%60)+'s';
 const rows=(players.length?players:party().map(ch=>({name:ch.name,class:ch.class,damage:Number(run?.damage?.[ch.id])||0,healing:Number(run?.healing?.[ch.id])||0,avoidableDamage:0}))).map(p=>'<div class="cbr-analysis-row"><i class="cb2d-dot '+classKey(p)+'"></i><span><b>'+esc(p.name||'Adventurer')+'</b><small>'+esc(p.class||'')+' · '+Math.round(Number(p.healing)||0).toLocaleString()+' healing · Avoidable '+Math.round(Number(p.avoidableDamage)||0).toLocaleString()+'</small></span><strong>'+Math.round(Number(p.damage)||0).toLocaleString()+' dmg</strong></div>').join('');
 return '<section class="cbr-analysis"><div class="cbr-analysis-head"><div><small>RUN ANALYSIS</small><h4>Grid execution and boss performance.</h4></div></div><div class="cbr-analysis-grid"><article><span>COMBAT TIME</span><b>'+time+'</b></article><article><span>DAMAGE</span><b>'+Math.round(damage).toLocaleString()+'</b></article><article><span>HEALING</span><b>'+Math.round(healing).toLocaleString()+'</b></article><article><span>PUZZLE MOVES</span><b>'+Number(run?.moves||0)+'</b></article><article><span>DEATHS</span><b>'+Number(metrics?.deaths||0)+'</b></article><article><span>FAILED MECHANICS</span><b>'+Number(metrics?.mechanicsFailed||0)+'</b></article></div><div class="cbr-analysis-list">'+rows+'</div></section>'
}
async function complete(){
 ...
 e.innerHTML='<div class="cb2d-loot-wrap"><header class="cb2d-loot-head"><div><small>BLACKOUT STATION · '+esc(mode==='cellbound'?'CELLBOUND+'+tier:mode.toUpperCase())+' · CLEARED</small><h3>Grid Secured</h3><p>The generator hall falls silent. The station is powered, Calder is down, and the recovered equipment has been secured.</p></div><div class="cb2d-loot-complete">✓<span>DUNGEON<br>COMPLETE</span></div></header><div class="cb2d-loot-currency"><article><span>GOLD</span><b>+'+gold+'</b><small>Added to Guild treasury</small></article><article><span>RENOWN</span><b>+'+renown+'</b><small>Guild reputation earned</small></article><article><span>PARTY XP</span><b>+'+XP+'</b><small>Each adventurer</small></article><article><span>CELL SHARDS</span><b>+'+shards+'</b><small>Recovered from the grid</small></article><article><span>RUN SCORE</span><b>'+score+'</b><small>'+Math.round(timeMs/1000)+'s expedition</small></article></div>'+(unlocks?'<section class="eg-unlock-panel"><small>PROGRESSION</small><div>'+unlocks+'</div></section>':'')+'<section class="cb2d-loot-section"><div class="cb2d-loot-title"><span>GEAR ACQUIRED</span><small>Sent to Guild Bank</small></div><div class="cb2d-loot-gear">'+(gear?bsLootGearCard(gear):'<div class="cb2d-loot-empty">No equipment recovered.</div>')+'</div></section><section class="cb2d-loot-section cb2d-xp-section"><div class="cb2d-loot-title"><span>PARTY EXPERIENCE</span><small>Active party XP</small></div><div class="cb2d-xp-grid">'+gains.map(bsXpCard).join('')+'</div></section>'+(overrideDrop?'<section class="cb2d-loot-section bs-override-drop-section"><div class="cb2d-loot-title"><span>RARE UTILITY DROP</span><small>10% chance · separate from equipment loot</small></div><article class="bs-override-drop-card">'+gridOverrideArt(72)+'<div><small>RARE · UTILITY</small><h4>Grid Override Module</h4><p>5 / 5 uses · Automatically restores the Blackout Station grid after your first manual clear.</p><em>TRADEABLE · SENT TO GUILD BANK</em></div></article></section>':'')+'<section class="bs-run-summary"><article><b>'+run.moves+'</b><span>Puzzle moves</span></article><article><b>'+deaths+'</b><span>Deaths</span></article><article><b>'+failed+'</b><span>Failed mechanics</span></article></section>'+bsCombatAnalysisHTML(metrics,timeMs)+'<footer class="cb2d-loot-actions"><button data-bs-bank>VIEW GUILD BANK</button><button class="primary" data-bs-return>RETURN HOME →</button></footer></div>';
 bsAnimateXp(e);e.querySelector('[data-bs-bank]').onclick=()=>{close();Game.switchView?.('bank')};e.querySelector('[data-bs-return]').onclick=()=>{close();Game.switchView?.('overview')};
 renderCard()
}
