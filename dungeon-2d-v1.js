...
let tactics={aggression:'balanced',interrupts:'important',interruptAssignment:'dps-rotation',cooldowns:'difficult',cc:'priority-elites',bossPlan:'balanced',defensives:'balanced',adds:'dangerous',movement:'balanced',consumables:'danger'};
...
function briefing(){
 ...
 r.innerHTML='<section class="cb2d-shell cb2d-brief"><header class="cb2d-head"><div><small>THE ASHEN VAULT · LEVELS 3–5 · ILVL 18+ · TACTICAL BRIEFING</small><h2>Set the plan once. Then watch the dungeon run.</h2></div><button data-close>×</button></header><div class="cb2d-brief-grid"><main><p class="cb2d-intro">These tactics persist through the whole expedition. The party will move, react and fight automatically. Difficulty changes the same Combat Reborn simulation rather than loading a separate combat model.</p>'+endgamePrepMarkup()+'<h3>Pull Style</h3>'+groupButtons('aggression',[['safe','SAFE','Slower route, stronger pull control.'],['balanced','BALANCED','Standard dungeon pace and pressure.'],['aggressive','AGGRESSIVE','Faster route with rougher opening threat.']])+'<h3>Cooldown Use</h3>'+groupButtons('cooldowns',[['bosses','SAVE FOR BOSSES','Hold long cooldowns on ordinary packs.'],['difficult','DIFFICULT PACKS','Spend cooldowns when pressure rises.'],['free','USE FREELY','Maximise uptime and speed.']])+'<h3>Interrupt Priority</h3>'+groupButtons('interrupts',[['important','IMPORTANT','Stop dangerous casts.'],['high','HIGH','Interrupt aggressively.'],['conservative','DANGER ONLY','Save interrupts for the most dangerous casts.']])+'<h3>Interrupt Assignment</h3>'+groupButtons('interruptAssignment',[['tank','TANK PRIORITY','Tank takes the first available interrupt.'],['dps-rotation','DPS ROTATION','Damage dealers rotate interrupts.'],['best','BEST AVAILABLE','Use the cleanest interrupt available.']])+'<h3>Crowd Control</h3>'+groupButtons('cc',[['priority-elites','PRIORITY ELITES','Control dangerous elites and first adds.'],['enabled','ENABLED','Use control on ordinary packs.'],['disabled','DISABLED','Do not spend control.']])+'<h3>Boss Strategy</h3>'+groupButtons('bossPlan',[['control','CONTROL','Prioritise adds, interrupts and survival.'],['balanced','BALANCED','No extreme commitment.'],['burn','BURN','Spend cooldowns freely and pressure the boss.']])+'<h3>Defensives</h3>'+groupButtons('defensives',[['early','EARLY','Use cooldowns sooner.'],['balanced','BALANCED','React to pressure.'],['save','SAVE','Hold for late danger.']])+'<h3>Add Priority</h3>'+groupButtons('adds',[['dangerous','DANGEROUS','Swap to threatening adds.'],['full','FULL','Clear every add wave.'],['boss','BOSS','Stay on primary target.']])+'<h3>Movement Discipline</h3>'+groupButtons('movement',[['safety','SAFETY FIRST','Move early and protect the run.'],['balanced','BALANCED','Respect mechanics without giving up free damage.'],['damage','MAX DAMAGE','Move later to preserve uptime.']])+'</main><aside><small>ACTIVE FIVE · PARTY LV '+partyLevel()+' · ILVL '+ilvl()+'</small>'+party().map(c=>'<div class="cb2d-brief-member"><i class="cb2d-dot '+classKey(c)+'"></i><span><b>'+esc(c.name)+'</b><small>Lv. '+Math.max(1,Number(c.level)||1)+' · '+esc(c.class)+' · '+esc(c.spec)+'</small></span><strong>'+String(role(c)).toUpperCase()+'</strong></div>').join('')+'<div class="cb2d-prep-summary"><small>ACTIVE PROFESSION PREP</small>'+party().map(c=>{const fx=P?.activeEffects?.(c)||[];return fx.length?'<p><b>'+esc(c.name)+'</b><span>'+fx.map(x=>esc(x.name)+' · '+x.remainingBosses+' bosses').join('<br>')+'</span></p>':''}).join('')+'</div><button class="cb2d-start" data-start>BEGIN EXPEDITION →</button></aside></div></section>';
 r.querySelector('[data-close]').onclick=close;
