...
function initialState(){
  return {saveVersion:SAVE_VERSION,gearVersion:2,renown:120,gold:1840,roster:JSON.parse(JSON.stringify(starterRoster)),party:{tank:'r1',healer:'r2',dps:['r3','r4','r5']},bossKills:{ashwarden:false,embermaw:false,vaultheart:false},reports:[],bank:[],collectionHistory:[],activity:['The guild charter has been signed.','Your first five adventurers are ready.','Tier 1 equipment issued to the active party.','The Ashen Vault is available.']};
}
...
function resolveAttempt(boss,party,success,result){
  ...
  if(success){state.bossKills[boss.id]=true;state.renown+=25;state.gold+=120;loot=canonicalItem(G.rollDungeonLoot(boss.name,boss.tier2Chance));addBankItem({...loot,source:boss.name});state.activity.push(`${loot.name} (${loot.tierLabel}, iLvl ${loot.itemLevel}) dropped from ${boss.name}.`);state.activity.push(`${boss.name} was defeated in The Ashen Vault.`);}else{party.forEach(c=>applyCellShock(c,PVE_WIPE_CELL_SHOCK));state.activity.push(`The guild wiped on ${boss.name}. Each participating character gained ${PVE_WIPE_CELL_SHOCK}% Cell Shock.`);}
  state.reports.unshift({id:Date.now(),boss:boss.id,success,knowledgeGain,loot:loot?.name||null,lootItemId:loot?.itemId||null,lootTier:loot?.tier||null,lootItemLevel:loot?.itemLevel||null,cellShockGain:success?0:PVE_WIPE_CELL_SHOCK,partyItemLevel:party.reduce((s,c)=>s+characterItemLevel(c),0)/5,at:new Date().toISOString()});save();renderAll();
  result.innerHTML=`<div class="attempt-result"><h3>${success?'VICTORY':'WIPE — CELL SHOCK GAINED'}</h3><p>${success?'Boss defeated. PvE victories do not reduce Cell Shock.':'The group wiped. Each participating character gained Cell Shock; encounter knowledge was still earned.'}</p>${loot?`<div class="loot-drop gear-loot-drop">${G.artHTML(loot,80)}<div><small>${loot.tierLabel.toUpperCase()} · ILVL ${loot.itemLevel} · ${loot.rarity.toUpperCase()} · ${loot.class}</small><b>${loot.name}</b><span>Stored in the Guild Bank</span></div></div>`:''}<p>${knowledgeGain.map(k=>`${k.name}: ${k.before}% → ${k.after}%${success?'':` · Shock ${charById(party.find(p=>p.name===k.name)?.id)?.cellShock||0}%`}`).join('<br>')}</p><button id="closeAttempt">RETURN TO GUILD</button></div>`;$('#closeAttempt')?.addEventListener('click',()=>ui.attemptModal.hidden=true);
}
...
function renderReports(){
  ...
  ui.reportsList.innerHTML=state.reports.map(r=>{const b=bossById(r.boss),loot=G.byId(r.lootItemId)||G.byName(r.loot);return `<article class="report-card"><div><div class="report-result ${r.success?'kill':'wipe'}">${r.success?'VICTORY':'WIPE'}</div><small>${new Date(r.at).toLocaleString()}</small></div><div><h3>${b?.name||'Encounter'}</h3><p>${r.success?'The party defeated the encounter. PvE victories do not clear Cell Shock.':`The party gained ${r.cellShockGain||PVE_WIPE_CELL_SHOCK}% Cell Shock and encounter knowledge.`}${loot?` Loot: ${loot.name} · iLvl ${r.lootItemLevel||loot.itemLevel||'—'} → Guild Bank.`:''}</p></div><div class="report-gain"><b>Knowledge gained</b>${r.knowledgeGain.map(k=>`<span>${k.name} +${k.gain}%</span>`).join('')}</div></article>`;}).join('');
}
