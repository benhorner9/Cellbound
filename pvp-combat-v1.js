...
function selfTest(){
  ...
  const a2=simulate({blue:blue.slice(1),red:red.slice(1),kind:'arena',mode:'arena',size:2,seed:'self-arena-2'}),a=simulate({blue,red,kind:'arena',mode:'arena',size:3,seed:'self-arena'}),a5=simulate({blue:[...blue,{...blue[2],id:'b4',name:'Blue DPS 2'},{...blue[2],id:'b5',name:'Blue DPS 3'}],red:[...red,{...red[2],id:'r4',name:'Red DPS 2'},{...red[2],id:'r5',name:'Red DPS 3'}],kind:'arena',mode:'arena',size:5,seed:'self-arena-5'}),b=simulate({blue:[...blue,...blue.map((x,i)=>({...x,id:'ba'+i,name:'Ally '+i}))],red:[...red,...red.map((x,i)=>({...x,id:'ra'+i,name:'Enemy '+i}))],kind:'battleground',mode:'king-of-the-hill',size:6,seed:'self-bg'}),c=simulate({blue:[...blue,...blue.map((x,i)=>({...x,id:'bc'+i,name:'Blue CTF '+i}))],red:[...red,...red.map((x,i)=>({...x,id:'rc'+i,name:'Red CTF '+i}))],kind:'battleground',mode:'capture-the-flag',size:6,seed:'self-ctf'});
  const flagEvents=c.events.filter(e=>e.type==='FLAG_STATE'),ctfMap=PVP_MAPS['cellwind-bastion'],mapCtx={map:ctfMap},kothMap=PVP_MAPS['shifting-court'],kothCtx={map:kothMap},arenaMap=PVP_MAPS['veilspire-arena'],arenaCtx={map:arenaMap},arenaRuns=[a2,a,a5];
  const arenaPhases=a.events.filter(e=>e.type==='ARENA_STATE'&&e.result==='storm-phase'),arenaProgress=a.events.filter(e=>e.type==='ARENA_STATE'&&e.result==='storm-progress'),arenaMoves=a.events.filter(e=>e.type==='MOVEMENT_START'&&/cellstorm|spread inside arena|rotate away/.test(String(e.result||'')));
  ...
  const tests=[
    ...
    {name:'Arena uses Veilspire terrain in 2v2 3v3 and 5v5',pass:arenaRuns.every(r=>r.map?.id==='veilspire-arena')&&Array.isArray(arenaMap?.blockers)&&arenaMap.blockers.length===4},
    {name:'Arena Cellstorm continuously shrinks and moves',pass:arenaPhases.length>=4&&arenaProgress.length>=20&&Number(arenaProgress[0]?.payload?.radius)>Number(arenaProgress.at(-1)?.payload?.radius)&&arenaProgress.some(e=>Math.abs(Number(e.payload?.x)-50)>1)},
    {name:'Arena players react to Cellstorm pressure',pass:arenaMoves.length>=3},
    {name:'Arena pillars block line of sight',pass:!hasLineOfSight(arenaCtx,{position:{x:30,y:36}},{position:{x:46,y:36}})},
    {name:'Arena Battle Fatigue prevents healer stalemates',pass:arenaRuns.every(r=>r.durationMs<45000||r.events.some(e=>e.type==='ARENA_STATE'&&e.result==='dampening'&&Number(e.payload?.dampeningPct)>=20))},
