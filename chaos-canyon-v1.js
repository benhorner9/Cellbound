...
const STAGES=[
 ...
 {id:'vorran',title:'Archdruid Vorran',kind:'FINAL BOSS',combatKind:'final',level:11,enemyTypes:['boss'],enemyHealth:1900,enemies:['Archdruid Vorran'],mechanics:[{name:'Rejuvenation',type:'self-heal',duration:1900,healPct:.09}],phases:[{id:'vorran-close-one',name:'The Canyon Closes',atPct:75,damageScale:1.04},{id:'vorran-close-two',name:'Roots Close In',atPct:50,damageScale:1.08},{id:'vorran-unbound',name:'True Chaos',atPct:25,damageScale:1.18,allAttacksAoe:true,addMechanics:[{name:'Wild Wrath: Unbound',type:'circles',duration:1200}]}]}
];
const ROUTE=[
...
function ccRebornEncounter(s){const base={id:s.id,title:s.title,kind:s.combatKind||'trash',level:s.level||1,recommendedItemLevel:s.level<=9?30:s.level===10?32:34,enemyLevels:s.enemyLevels||null,enemyTypes:s.enemyTypes||null,enemies:[...s.enemies],enemyHealth:s.enemyHealth,mechanics:(s.mechanics||[]).map(m=>Array.isArray(m)?{name:m[0],type:m[1],duration:m[2]}:{...m}),phases:(s.phases||[]).map(p=>({...p,addMechanics:(p.addMechanics||[]).map(m=>({...m}))}))};let cfg=window.CellboundEndgame?.stageConfig?.('chaos-canyon',base)||base;cfg={...cfg,scaling:{...(cfg.scaling||{})}};if(s.id==='warden'&&run?.chaosScar){const scar=Math.min(40,Math.max(0,Number(run.chaosScar)||0)*5);cfg.scaling.enemyDamage=(Number(cfg.scaling.enemyDamage)||1)*(1+scar/100);cfg.chaosScarPct=scar}return cfg}
...
function ccCastStart(name,duration){
 const panel=$('#cc2dCastPanel'),label=$('#cc2dCastName'),time=$('#cc2dCastTime'),fill=$('#cc2dCastFill');if(panel)panel.hidden=false;if(label)label.textContent=name||'Enemy cast';if(time)time.textContent=((Number(duration)||0)/1000).toFixed(1)+'s';if(fill){fill.style.transition='none';fill.style.width='0%';void fill.offsetWidth;fill.style.transition='width '+Math.max(.1,(Number(duration)||500)/1000/(run?.speed||1))+'s linear';fill.style.width='100%'}
}
...
function ccAnimateXp(rootEl){
 [...(rootEl?.querySelectorAll('[data-cc-xp]')||[])].forEach((row,index)=>{const bar=row.querySelector('.cb2d-xp-bar i'),end=Number(row.dataset.end)||0,levels=Number(row.dataset.levels)||0;if(!bar)return;setTimeout(()=>{if(!levels){bar.style.width=end+'%';return}bar.style.width='100%';setTimeout(()=>{row.classList.add('levelled');bar.style.transition='none';bar.style.width='0%';void bar.offsetWidth;bar.style.transition='width .8s cubic-bezier(.2,.75,.25,1)';bar.style.width=end+'%'},760)},220+index*90)})
}
