...
function briefing(){
 ...
 if(!gate.ok){r.innerHTML='<section class="cb2d-shell cb2d-brief cc2d-unified-shell"><header class="cb2d-head"><div><small>CHAOS CANYON · ENTRY CHECK</small><h2>Dungeon entry is currently blocked.</h2></div><button data-close aria-label="Close dungeon">×</button></header><div class="cb2d-blocked"><b>NOT READY</b><p>'+esc(gate.reason)+'</p><button data-action>OPEN PARTY BUILDER →</button></div></section>';r.querySelector('[data-close]').onclick=close;r.querySelector('[data-action]').onclick=()=>{close();Game.switchView?.('party')};return}
...
async function complete(){
 ...
 ccAnimateXp(end);end.querySelector('[data-cc-replay]')?.addEventListener('click',ccReplayFinalFight);end.querySelector('[data-loot-bank]').onclick=()=>{close();Game.switchView?.('bank')};end.querySelector('[data-loot-return]').onclick=()=>{close();Game.switchView?.('overview')}
}
